        ...
        children: [
          ...
          SingleChildScrollView(
            child: Container(
              child: Column(
                children: [
                  // CustomTextField2('Enter current email', context,
                  //     onChanged: (val) {
                  //   _emailcontroller.email = val;
                  // }),
                  SizedBox(height: size.convert(context, 80)),
                  CustomTextField2('Enter new email', context,
                      onChanged: (val) {
                    _emailcontroller.newEmail = val;
                  }),
                  // CustomTextField2('Password', context, onChanged: (val) {
                  //   _emailcontroller.password = val;
                  // }),
                  SizedBox(
                    height: size.convert(context, 30),
                  ),
                  SubmitButton(context, 'submit', () {
                    _emailcontroller.id = userid;
                    _emailcontroller.emmailupdate();
                  })
                ],
              ),
            ),
